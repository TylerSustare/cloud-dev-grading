describe("hw1", () => {
  ...
  beforeEach(async () => {
    ...
    await browser.on("targetcreated", async () => {
      ...
      await newPage.screenshot({
        path: "./hw1-02-click-signin-with-google.png"
      });

      await newPage.waitForSelector(`#identifierId`);
      await newPage.type(`#identifierId`, GOOGLE_USER, { delay: 5 });
      await newPage.screenshot({
        path: "./hw1-03-signin-with-google-user.png"
      });
      await newPage.click("#identifierNext");
      await newPage.waitFor(1000);

      await newPage.screenshot({
        path: "./hw1-04-signin-with-google-password.png"
      });
      await newPage.waitForSelector(`#password input[type="password"]`);
      await newPage.type(`#password input[type="password"]`, GOOGLE_PASSWORD, {
        delay: 5
      });
  ...
  describe("60 Points - Auth tests", () => {
    test.skip("20 Points - Can sign in and out with Google (signInWithPopup Firebase Method)", async () => {
      maxGrade += 20;
      console.log(
        "You need a very insecure test account to run this automated test."
      );
      ...
      await page.waitFor(16000); /* wait for popup event to finish */
    ...
    test("20 Points - Can sign up with email & password & Sign out", async () => {
      ...
      await page.screenshot({
        path: "./hw1-08-signup-with-email-password.png"
      });
      ...
      await page.screenshot({
        path: "./hw1-09-signup-with-email-password.png"
      });
      grade += 15;
    ...
    test("20 Points - Can sign in with email & password created in previous test", async () => {
      ...
      await page.type("#password", password, { delay: 5 });
      await page.screenshot({
        path: "./hw1-12-signin-with-email-password.png"
      });
      ...
      expect(text).toEqual(`Hello, ${email}`);
      await page.screenshot({
        path: "./hw1-13-signin-with-email-password.png"
      });
      grade += 15;
